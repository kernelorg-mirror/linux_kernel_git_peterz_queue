...
restart:
	scoped_guard (rcu) {
		list_for_each_entry_rcu(iter, &event->rb->event_list, rb_entry) {
			/*
			 * For per-CPU events, we need to make sure that
			 * neither they nor their children are running; for
			 * cpu==-1 events it's sufficient to stop the event
			 * itself if it's active, since it can't have children.
			 */
			cpu = iter->cpu;
			if (cpu == -1)
			     ...
			     struct list_head *filters)
{
	...
	filters = no_free_ptr(fguard); // allow filters to escape to the caller
	return 0;
}
