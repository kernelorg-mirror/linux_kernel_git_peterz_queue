...
static void __init_or_module poison_endbr(void *addr, struct module *mod, bool warn)
{
...
void __init_or_module noinline apply_seal_endbr(s32 *start, s32 *end, struct module *mod)
{
	...
	for (s = start; s < end; s++) {
		...
		poison_endbr(addr, mod, true);
		if (IS_ENABLED(CONFIG_FINEIBT))
			poison_cfi(addr - 16, mod);
	}
}
...
static void cfi_rewrite_endbr(s32 *start, s32 *end, struct module *mod)
{
	...
	for (s = start; s < end; s++) {
		...
		poison_endbr(addr + 16, mod, false);
	}
}
...
static void poison_cfi(void *addr, struct module *mod)
{
	...
	case CFI_FINEIBT:
		 ...
		 * 1:	nop
		 */
		poison_endbr(addr, mod, false);
		poison_hash(addr + fineibt_preamble_hash, mod);
		break;
